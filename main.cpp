...
void FlagUnrecognizedCharacters(const std::string &line) {
    std::regex unrecognizedRgx("[^a-zA-Z0-9_\\s]");
    std::smatch match;
    if (std::regex_search(line, match, unrecognizedRgx)) {
        SetColor(FOREGROUND_RED);
        std::cout << "Unrecognized character(s) in line: " << match[0] << std::endl;
...
int main() {
    ...
    while (std::getline(inFile, line)) {
        ...
        std::smatch match;
        FlagUnrecognizedCharacters(line);
